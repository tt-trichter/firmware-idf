...
static int trichter_status_access(
    uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg
);
static int trichter_result_access(
    uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg
);
static int trichter_control_access(
    uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg
);
static int trichter_image_access(
    uint16_t conn_handle, uint16_t attr_handle, struct ble_gatt_access_ctxt *ctxt, void *arg
);
...
)
{
    ...
    switch (ctxt->op) {
        case BLE_GATT_ACCESS_OP_READ_CHR:
            TRICHTER_LOGD(TAG, "Status characteristic read; conn_handle=%u", conn_handle);
            rc = os_mbuf_append(
                ctxt->om,
        ...
        default:
            TRICHTER_LOGE(TAG, "Unexpected operation on status characteristic: %d", ctxt->op);
...
)
{
    ...
    switch (ctxt->op) {
        case BLE_GATT_ACCESS_OP_READ_CHR:
            TRICHTER_LOGD(TAG, "Result characteristic read; conn_handle=%u", conn_handle);

            if (ble_state.current_result != NULL) {
                trichter_ble_result_t ble_result = {
                    .duration_ms = (uint32_t)(ble_state.current_result->duration_us / 1000ULL),
                    ...
                    .image_size = ble_state.current_result->image_fb
                                      ...
                                      : 0,
                    .image_width = ble_state.current_result->image_fb
                                       ? (uint16_t)ble_state.current_result->image_fb->width
                                       : 0,
                    .image_height = ble_state.current_result->image_fb
                                        ? (uint16_t)ble_state.current_result->image_fb->height
                                        : 0,
                    .image_format = ble_state.current_result->image_fb
        ...
        default:
            TRICHTER_LOGE(TAG, "Unexpected operation on result characteristic: %d", ctxt->op);
            return BLE_ATT_ERR_UNLIKELY;
    }
}
...
)
{
    switch (ctxt->op) {
        case BLE_GATT_ACCESS_OP_WRITE_CHR:
            TRICHTER_LOGD(TAG, "Control characteristic write; conn_handle=%u", conn_handle);
        ...
        default:
            TRICHTER_LOGE(TAG, "Unexpected operation on control characteristic: %d", ctxt->op);
            return BLE_ATT_ERR_UNLIKELY;
    }
}
...
static void trichter_ble_prepare_image_transfer(const SessionResult *result)
{
    ...
    if (!result || !result->image_fb || !result->image_fb->buf || result->image_fb->len == 0) {
        TRICHTER_LOGI(TAG, "No image data available for transfer");
        return;
    }

    TRICHTER_LOGI(TAG, "3. Image timestamp: %d", result->image_fb->timestamp.tv_sec);
    uint8_t *copy = malloc(result->image_fb->len);
    if (!copy) {
        TRICHTER_LOGE(
            TAG,
            "Failed to allocate image transfer buffer: %zu bytes",
            result->image_fb->len
        );
        return;
    }
    ...
    TRICHTER_LOGI(
        ...
        image_tx.size,
        result->image_fb->width,
        result->image_fb->height,
...
static void trichter_ble_start_image_transfer(void)
{
    ...
    if (!ble_state.image_subscribed) {
        TRICHTER_LOGW(TAG, "Cannot start image transfer: image notifications not subscribed");
        return;
    }
    ...
    TRICHTER_LOGI(
        TAG,
        "Starting image transfer: id=%lu size=%zu conn=%u",
        (unsigned long)image_tx.transfer_id,
        image_tx.size,
...
static void trichter_ble_cancel_image_transfer(void)
{
    if (image_tx.active) {
        TRICHTER_LOGI(
            TAG,
            "Cancelling active image transfer: id=%lu offset=%zu/%zu",
            (unsigned long)image_tx.transfer_id,
            image_tx.offset,
            image_tx.size
        );
    } else {
        TRICHTER_LOGI(TAG, "Image transfer cancel requested, but no transfer active");
    }
...
static void trichter_ble_send_next_image_chunk(void)
{
    ...
    if (!ble_state.image_subscribed) {
        TRICHTER_LOGW(TAG, "Cannot send image chunk: image notifications not subscribed");
        image_tx.active = false;
        return;
    }
    ...
    if (max_packet <= header_size) {
        TRICHTER_LOGE(TAG, "MTU too small for image transfer: mtu=%u header=%zu", mtu, header_size);
        image_tx.active = false;
        return;
    }
    ...
    int rc = ble_gatts_notify_custom(ble_state.conn_handle, image_char_handle, om);
    if (rc != 0) {
        TRICHTER_LOGE(
            TAG,
    ...
    TRICHTER_LOGI(
        ...
        image_tx.offset,
        chunk_len,
        image_tx.size - (image_tx.offset + chunk_len),
        hdr.flags,
        mtu
    );
    ...
    if (image_tx.offset >= image_tx.size) {
        TRICHTER_LOGI(
            TAG,
            "Image transfer complete: id=%lu total=%zu",
            (unsigned long)image_tx.transfer_id,
            image_tx.size
        );
        image_tx.active = false;
...
void trichter_ble_notify_status(void)
{
    ...
    if (rc != 0) {
        TRICHTER_LOGW(TAG, "Failed to send periodic status notification: %d", rc);
    }
}
...
void trichter_ble_set_status(trichter_session_status_t status)
{
    ...
    TRICHTER_LOGI(
        TAG,
        "Updating trichter ble_state: %s -> %s",
        trichter_status_to_string(ble_state.current_status),
        trichter_status_to_string(status)
    );

    ble_state.current_status = status;
    trichter_ble_notify_status();
}
...
void trichter_ble_send_result(const SessionResult *result)
{
    ...
    TRICHTER_LOGI(
        TAG,
        "Connected=%d result_subscribed=%d image_subscribed=%d",
        ble_state.connected,
    ...
    if (ble_state.connected && ble_state.result_subscribed) {
        ...
        if (rc == 0) {
            TRICHTER_LOGI(
                ...
                result->duration_us / 1e6f,
                result->rate_lpm,
                result->volume_l,
                result->image_fb != NULL ? 1 : 0,
        ...
        TRICHTER_LOGW(
            TAG,
            "Result not indicated: connected=%d subscribed=%d",
            ble_state.connected,
            ble_state.result_subscribed
        );
    }
...
esp_err_t trichter_ble_service_init(void)
{
    ...
    ble_state.current_status = TRICHTER_STATUS_WAITING;
...
{
    return ble_state.waiting_for_ack;
}
...
void trichter_ble_handle_subscribe(struct ble_gap_event *event)
{
    ...
    TRICHTER_LOGI(
        TAG,
        "[SUB] conn=%u handle=%u notify=%u indicate=%u",
    ...
    if (handle == status_char_handle) {
        ...
        TRICHTER_LOGI(TAG, "Status notifications %s", notify ? "enabled" : "disabled");
...
void trichter_ble_print_state(void)
{
    ...
    TRICHTER_LOGI(TAG, "Status Subscribed: %s", ble_state.status_subscribed ? "YES" : "NO");
    TRICHTER_LOGI(TAG, "Result Subscribed: %s", ble_state.result_subscribed ? "YES" : "NO");
    TRICHTER_LOGI(TAG, "Image Subscribed: %s", ble_state.image_subscribed ? "YES" : "NO");
    TRICHTER_LOGI(TAG, "Waiting for ACK: %s", ble_state.waiting_for_ack ? "YES" : "NO");
    TRICHTER_LOGI(TAG, "Current Result: %s", ble_state.current_result ? "AVAILABLE" : "NULL");

    TRICHTER_LOGI(TAG, "Image Transfer Active: %s", image_tx.active ? "YES" : "NO");
    TRICHTER_LOGI(TAG, "Image Transfer ID: %lu", (unsigned long)image_tx.transfer_id);
