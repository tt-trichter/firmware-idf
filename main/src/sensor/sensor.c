...
#define PULSES_PER_LITER (FLOW_HZ_PER_LPM * 60.0f)
...
esp_err_t sensor_init(gpio_num_t pulse_gpio)
{
    ...
    TRICHTER_CHECK(s_first_sem && s_idle_sem, TRICHTER_ERR_MEMORY, "Failed to create semaphores");
    ...
    TRICHTER_CHECK_ERR(gpio_config(&io_conf), TRICHTER_ERR_SENSOR, "Failed to configure GPIO");
    TRICHTER_CHECK_ERR(
    ...
    pcnt_unit_config_t unit_cfg =
        {.low_limit = -1, .high_limit = INT16_MAX, .intr_priority = 1, .flags.accum_count = 1};
    TRICHTER_CHECK_ERR(
    ...
    TRICHTER_CHECK_ERR(
        ...
        TRICHTER_ERR_SENSOR,
        "Failed to set glitch filter"
    );

    pcnt_chan_config_t chan_cfg = {.edge_gpio_num = pulse_gpio, .level_gpio_num = -1, .flags = {0}};
    TRICHTER_CHECK_ERR(
        pcnt_new_channel(s_pcnt_unit, &chan_cfg, &s_pcnt_chan),
        TRICHTER_ERR_SENSOR,
...
esp_err_t sensor_arm(void)
{
    ...
    s_armed = true;
    TRICHTER_LOGI(TAG, "Sensor armed — waiting for first pulse on GPIO %d", s_pulse_gpio);
    return ESP_OK;
}
...
esp_err_t sensor_measure_session(SessionResult *out_result)
{
    ...
    uint64_t startup_deadline = t_start + (uint64_t)CONFIG_SENSOR_STARTUP_WINDOW_MS * 1000ULL;

    uint64_t now = esp_timer_get_time();
    int64_t remaining_us = (int64_t)startup_deadline - (int64_t)now;
    TRICHTER_LOGI(
        TAG,
    ...
    int startup_count = 0;
    TRICHTER_CHECK_ERR(
        pcnt_unit_get_count(s_pcnt_unit, &startup_count),
        TRICHTER_ERR_SENSOR,
        "Failed to get PCNT count after startup window"
    );
    ...
    TRICHTER_LOGI(TAG, "Startup gate PASSED: %d pulses — session running", startup_count);
    ...
    while (true) {
        ...
        if (cnt != last_count) {
            last_count = cnt;
            t_last_pulse = esp_timer_get_time();
    ...
    int total_pulses;
    TRICHTER_CHECK_ERR(
        pcnt_unit_get_count(s_pcnt_unit, &total_pulses),
        TRICHTER_ERR_SENSOR,
        "Failed to get final PCNT count"
    );

    uint64_t dur_us = t_last_pulse - t_start;
    float secs = dur_us / 1e6f;
    float rate_lpm = ((float)total_pulses / secs) / FLOW_HZ_PER_LPM;
    float volume_l = (float)total_pulses / PULSES_PER_LITER;
    ...
    TRICHTER_LOGI(
        TAG,
        "Result: %d pulses, %.2fs, %.2f L/min, %.2f L, image: %s",
        total_pulses,
        secs,
        rate_lpm,
        volume_l,
    ...
    TRICHTER_LOGI(
        TAG,
        "Result: %d pulses, %.2fs, %.2f L/min, %.2f L",
        total_pulses,
        secs,
        rate_lpm,
        volume_l
