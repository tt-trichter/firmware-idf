...
static esp_err_t initialize_system_components(void)
{
    ...
    TRICHTER_CONFIG_CHECK(TRICHTER_SENSOR_ENABLED, "Sensor must be enabled for this firmware");

    ret = nvs_flash_init();
    if (ret == ESP_ERR_NVS_NO_FREE_PAGES || ret == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        TRICHTER_CHECK_ERR(nvs_flash_erase(), TRICHTER_ERR_INIT, "Failed to erase NVS flash");
        ret = nvs_flash_init();
    }
    ...
    if (trichter_is_sensor_available()) {
        ...
        if (ret == ESP_OK) {
            ...
            TRICHTER_LOGE(TAG, "Failed to initialize sensor: %s", esp_err_to_name(ret));
            return ret;
        }
    }
    ...
    if (trichter_is_camera_available()) {
        ...
        if (ret == ESP_OK) {
            ...
            TRICHTER_LOGE(TAG, "Failed to initialize camera: %s", esp_err_to_name(ret));
...
static void process_session_result(SessionResult *session_result)
{
    ...
    if (app_ctx.ble_enabled && trichter_ble_is_connected()) {
        ...
        while (trichter_ble_is_waiting_for_ack() && trichter_ble_is_connected()) {
    ...
    s_last_displayed = APP_STATE_INIT; // force display redraw on next WAITING_SESSION
...
static void run_fake_session(SessionResult *session_result)
{
    ...
    if (app_ctx.camera_enabled) {
        ...
        if (session_result->image_fb) {
            TRICHTER_LOGI(
                TAG,
                "Camera took picture! Size: %dx%d",
                session_result->image_fb->width,
                session_result->image_fb->height
            );
        } else {
            TRICHTER_LOGW(TAG, "Fake session image capture failed");
        }
    }
#endif
        ...
        case TRICHTER_CMD_FAKE_RUN:
            if (app_ctx.state == APP_STATE_WAITING_SESSION) {
                ...
                if (app_ctx.state == APP_STATE_WAITING_SESSION) {
                    ...
                    TRICHTER_LOGW(
                        TAG,
                        "Ignoring fake run request: state=%s",
...
static esp_err_t initialize_ble_stack(void)
{
    ...
    if (ret != ESP_OK) {
        TRICHTER_LOGE(TAG, "Failed to initialize nimble stack: %s", esp_err_to_name(ret));
        return ret;
    }
...
static void update_ble_connection_state(void)
{
    ...
    if (app_ctx.ble_connected && !was_connected) {
        ...
        TRICHTER_LOGI(TAG, "BLE client disconnected - switching to standalone mode");
        set_state(APP_STATE_WAITING_SESSION);
    }
}
...
void app_main(void)
{
    ...
    if (init_ret != ESP_OK) {
        TRICHTER_LOG_ERROR(TRICHTER_ERR_INIT, init_ret, "System components initialization failed");
    ...
    if (led_ret != ESP_OK) {
        TRICHTER_LOGW(
            TAG,
            "LED status init failed: %s — continuing without LED",
    ...
    set_state(APP_STATE_WAITING_SESSION);
    SessionResult session_result = {0};

    TRICHTER_LOGI(TAG, "Creating fake task...");
    BaseType_t rc = xTaskCreate(fake_run_task, "fake_run", 4 * 1024, NULL, 5, &s_fake_run_task);
    if (rc != pdPASS) {
    ...
    TRICHTER_LOGI(TAG, "Camera: %s", app_ctx.camera_enabled ? "enabled" : "disabled");
    TRICHTER_LOGI(TAG, "Display: %s", app_ctx.display_enabled ? "enabled" : "disabled");
    TRICHTER_LOGI(TAG, "Sensor: %s", app_ctx.sensor_enabled ? "enabled" : "disabled");
        ...
        switch (app_ctx.state) {
            case APP_STATE_WAITING_SESSION:
                ...
                if (app_ctx.sensor_enabled) {
                    esp_err_t arm_err = sensor_arm();
                    if (arm_err != ESP_OK) {
                        TRICHTER_LOGE(TAG, "Failed to arm sensor: %s", esp_err_to_name(arm_err));
                        set_state(APP_STATE_ERROR);
                        break;
                    }
                    ...
                    if (sensor_poll_triggered()) {
                        ...
                        if (err != ESP_OK) {
                            TRICHTER_LOGE(
                                TAG,
                                "Session failed: %s — re-arming sensor",
                                esp_err_to_name(err)
                            );
