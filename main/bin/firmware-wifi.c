...
static esp_err_t submit_session_to_server(const SessionResult *session_result) {
  ...
  session_data_t session_data = {.rate = session_result->rate_lpm,
                                 .duration = session_result->duration_us / 1e6f,
                                 .volume = session_result->volume_l,
                                 .image_fb = session_result->image_fb};

  ESP_LOGI(TAG,
           "Submitting session to server: Rate=%.2f L/min, Duration=%.2fs, "
           "Volume=%.2f L",
  ...
  if (err == ESP_OK) {
    ...
    ESP_LOGE(TAG, "Failed to submit session to server: %s",
             esp_err_to_name(err));
  }
